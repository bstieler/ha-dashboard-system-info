...
- **STG-13.20** — PULSE M3: Energy view migration (ha-dashboard-energy topics → PULSE view) (`none`)
...
- **STG-4.155** — Accuracy-Karten: Phantom-Punkt für aktuellen Tag entfernen (extend_to) (`yaml_reload`)
...
- **STG-2.405** — Target timeline: collapse restart/boot transients and flip-flop islands (`core_restart`)
...
- **STG-3.134** — MOS dawn bias adds phantom generation on zero-irradiance night hours (`core_restart`)
...
- **STG-8.8** — Circulation pump: remove leftover hardcoded plug references after SmartPlug abstraction (`yaml_reload`)
...
- **STG-2.345** — Crossover Forecast: morgiger Cross-over wird nicht berechnet (`core_restart`)
...
- **STG-4.120** — Pump status chart: reduce graph span to 6h for readable resolution (`yaml_reload`)
...
- **STG-12.47** — kvv_trip: POI and locality searches find no connection — wrong EFA type mapping (`core_restart`)
...
- **STG-12.41** — kvv-search-card: expandable alternatives in the same style as the main connection (`yaml_reload`)
...
- **STG-12.40** — kvv-search-card: favourites cannot be deleted (`yaml_reload`)
...
- **STG-2.297** — Night plan consumption revision: structural shrink of future-only forecast series triggers false mid-night re-anchors (`core_restart`)
...
- **STG-4.95** — ApexCharts standard template with house palette as shared YAML anchor (`yaml_reload`)
...
- **STG-4.94** — zs7 theme: central theme file with all design tokens (`yaml_reload`)
...
- **STG-1.9** — ha-forecast-common: gemeinsame SQLite-Connection-Basisklasse (`core_restart`)
...
- **STG-1.8** — ha-forecast-common: open_meteo_client.py zentralisieren (Divergenz aufloesen) (`core_restart`)
...
- **STG-4.92** — PV-Forecast dashboards: scatter 'Forecast vs Ist' renders years on x-axis (plotly date-axis default) (`yaml_reload`)
...
- **STG-2.277** — Day-Start-Phase (Halte-Niedrig) in Plankurve und Phasen-Tabelle darstellen (`core_restart`)
