...
This is a custom-component override that guards against a race condition in
HA Core where update() may be called before _under_voltage is initialized.
"""
...
class RaspberryChargerBinarySensor(BinarySensorEntity):
    ...
    def update(self) -> None:
        """Update the state."""
        # Workaround for HA Core race condition: _under_voltage may be None
        # if the entity is updated before the config entry runtime data is set.
        if self._under_voltage is None:
